...
def get_latest_launcher_version():
    ...
    try:
        ...
        if release_tag:
            ...
            # print(f"Latest portapps version on GitHub: {latest_version}")
...
def get_latest_qbittorrent_version():
    ...
    try:
        ...
        if version_span:
            ...
            # print(f"Latest qBittorrent version: {version}")
            return version
        else:
...
if "qbittorrent" not in current_folder.lower():
    print("Error: This script must be run from a folder containing 'qbittorrent' in its name. Make sure you have moved this script to right location.")
    input("\nPress Enter to exit...")
    sys.exit(1)
...
if portable_mode and launcher_update_needed:
    # ("Updating qbittorrent-portable.exe")
    ...
    if download_portable_launcher(launcher_version, temp_launcher_portable_exe):
        try:
            ...
            if os.path.exists(backup_launcher):
                try:
                    shutil.copy2(backup_launcher, launcher-portable_exe)
...
if standard_update_needed:
    ...
    for root, dirs, files in os.walk(exe_extract_folder):
        for file in files:
            if file.lower() == "qbittorrent.exe":
                ...
                # print("Found qbittorrent.exe in extracted files.")
                break
    ...
    if not qbittorrent_found:
        ...
        input("\nPress Enter to exit...")
        sys.exit(1)
    # Copy extracted files to app folder
    if os.path.exists(app_folder):
